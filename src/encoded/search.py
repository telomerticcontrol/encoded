...
def search(context, request, search_type=None, return_generator=False):
    ...
    search_view.query_es()
    search_view.update_result_with_es(context)
    search_view.search_result['notification'] = 'Success'
    if not search_view.search_result['total']:
        return search_view.response_no_results(return_generator)
    elif not search_view.do_scan:
        return search_view.response_no_scan(return_generator)
    else:
        search_view.query_es_scan_hits()
        if request.__parent__ is not None or return_generator:
            return search_view.response_graph(return_generator)
        return search_view.response_graph_iter()
