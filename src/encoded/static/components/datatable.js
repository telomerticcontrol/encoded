 ...
 *     css?: <string> - CSS classes to add to <td />
 *     style?: <object> - CSS styles (not classes) to add to <tr />
 ...
 * `testFunc` output, and the "3" appears lightweight. The first number in `contentFunc` output is
 * italic, the middle is bold, and the last has a colored background. Each <tr /> in the table has
 * a CSS class of "overall-row" except the third which has "row-class". The "7" in the second row's
 * <td /> has a css class of "cell-override", and the "5" in the third row is italic and the last
 * number has a yellow background. The third column of the third row is an empty <td /> because of the
 * null value.
 *
 * Notes:
 * - `rows` is the only required property of `tableData`. and it must be an array.
 * - Each element of `row` must be either an array of elements to display in a single row, or an
 *   object.
 * - Each element of `row` that's an object must have a `rowContent` property.
 * - The `rowContent` property must be an array of elements to display in a single row.
 */
