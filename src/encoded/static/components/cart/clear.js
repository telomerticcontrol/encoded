...
class CartClearModalComponent extends React.Component {
    ...
    render() {
        const { cartName, closeClickHandler } = this.props;
        return (
...
const CartClearButtonInternal = connect(CartClearButtonComponent.mapStateToProps)(CartClearButtonComponent);

const CartClearButton = (props, reactContext) => (
    <CartClearButtonInternal session={reactContext.session} fetch={reactContext.fetch} />
);

CartClearButton.contextTypes = {
    session: PropTypes.object,
    fetch: PropTypes.func,
};
